        ...
        for jogo in jogos_a_listar:
            ...
            try:
                ...
                if len(callback_data.encode('utf-8')) > 64:
                    M_full_short = M_full.split(' ')[0][:8]
                    V_full_short = V_full.split(' ')[0][:8]
                    
                    # CORREÇÃO: Garante que o nome curto também seja escapado
                    M_safe_short = escape_markdown(M_full_short)
                    V_safe_short = escape_markdown(V_full_short)
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.startswith("JOGO|"):
            ...
            # O nome no callback_data AGORA está sempre escapado (M_safe) ou escapado/encurtado (M_safe_short)
        ...
        try:
             # A mensagem de erro original que você recebeu
