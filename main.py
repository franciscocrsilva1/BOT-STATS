# ===============================================================================
# 🏆 BOT DE ESTATÍSTICAS DE CONFRONTO V2.0.0 - VERSÃO FINAL E FUNCIONAL COM PERSISTÊNCIA DE FILTRO
# ===============================================================================
...
async def mostrar_menu_acoes(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, mandante: str, visitante: str, filtro_idx: int = 0):
    ...
    keyboard = [
        # O botão Estatísticas vai para a tela de Filtros, mas agora ele também carrega o filtro_idx
        # A tela de filtros (mostrar_menu_filtros_confronto) aplicará o filtro_idx
        [InlineKeyboardButton("📊 Estatísticas Detalhadas", 
                              callback_data=f"AÇÃO|STATS|{aba_code}|{m_sanitized}|{v_sanitized}|{filtro_idx}")],
        
        # O botão Resultados já chama a função de exibição, passando o filtro_idx
        [InlineKeyboardButton("📜 Últimos Resultados", 
                              callback_data=f"AÇÃO|RESULT|{aba_code}|{m_sanitized}|{v_sanitized}|{filtro_idx}")],
...
async def mostrar_menu_filtros_confronto(update: Update, context: ContextTypes.DEFAULT_TYPE, tipo_acao: str, aba_code: str, mandante: str, visitante: str, filtro_idx_padrao: int = 0):
    ...
    # Voltar para o Menu de Ações (Estatísticas/Resultados) - USA O FILTRO PADRÃO
    keyboard.append([InlineKeyboardButton("⬅️ Voltar para Ações", callback_data=f"AÇÃO_MENU|{aba_code}|{m_sanitized}|{v_sanitized}|{filtro_idx_padrao}")])
    reply_markup = InlineKeyboardMarkup(keyboard)
...
# CORREÇÃO CRÍTICA AQUI: O botão de Voltar para Ações inclui o filtro_idx
async def exibir_estatisticas(update: Update, context: ContextTypes.DEFAULT_TYPE, mandante: str, visitante: str, aba_code: str, filtro_idx: int):
    ...
    # CORREÇÃO: O botão de voltar para ações precisa levar o filtro_idx atual
    keyboard.append([InlineKeyboardButton("⬅️ Voltar para Ações", callback_data=f"AÇÃO_MENU|{aba_code}|{m_sanitized}|{v_sanitized}|{filtro_idx}")])
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if tipo_comando == "c": # Ex: c|BSA
            if not args:
                await query.edit_message_text("Erro: Comando 'c' sem argumento de liga.")
                return
            liga = args[0]
            ...
            if len(args) < 2:
                await query.edit_message_text("Erro: Comando 'STATUS' sem argumentos suficientes.")
                return
            ...
            if len(args) < 3:
                await query.edit_message_text("Erro: Comando 'JOGO' sem argumentos suficientes.")
                return
            liga, mandante, visitante = args
            
            # Ao escolher um novo jogo, o filtro padrão é 0 (GERAL)
            await mostrar_menu_acoes(update, context, liga, mandante, visitante, filtro_idx=0)
        
        elif tipo_comando == "AÇÃO": # Ex: AÇÃO|STATS|BSA|M|V|0
            # AÇÃO sempre terá 5 argumentos com a correção (incluindo filtro_idx)
            if len(args) < 5:
                # Fallback: Se faltar o filtro_idx (código antigo), usa o 0
                acao_tipo, liga, mandante, visitante = args[:4]
                filtro_idx = 0 
            else:
                acao_tipo, liga, mandante, visitante, filtro_idx_str = args
                filtro_idx = safe_int(filtro_idx_str)
            
            # O fluxo de STATS vai para a função de exibição para aplicar o filtro_idx e mostrar o menu de filtros
            if acao_tipo == "STATS":
                await exibir_estatisticas(update, context, mandante, visitante, liga, filtro_idx)

            # O fluxo de RESULT usa o filtro_idx
            ...
            if len(args) < 5:
                await query.edit_message_text("Erro: Comando 'FILTRO' sem argumentos suficientes.")
                return
            acao_tipo, liga, mandante, visitante, filtro_idx_str = args
            filtro_idx = safe_int(filtro_idx_str)
        ...
        # --- FLUXOS DE VOLTAR ---
            ...
            if not args:
                 await listar_competicoes(update, context) # Volta para a tela inicial se o argumento falhar
                 return
            liga = args[0]
            await mostrar_menu_status_jogo(update, context, liga)

        elif tipo_comando == "AÇÃO_MENU": # Ex: AÇÃO_MENU|BSA|M|V|1
             if len(args) < 3:
                 await listar_competicoes(update, context) 
                 return
             
             liga, mandante, visitante = args[:3]
             # Argumento opcional do filtro (padrão é 0)
             filtro_idx = safe_int(args[3]) if len(args) >= 4 else 0
             
             await mostrar_menu_acoes(update, context, liga, mandante, visitante, filtro_idx)

        else:
            await query.edit_message_text(f"Comando desconhecido: {data}")
            
    except Exception as e:
        # Loga qualquer erro de execução interna, mas falha elegantemente no Telegram
        logging.error(f"Erro no callback_query_handler para '{data}': {e}")
        try:
            await query.edit_message_text("❌ Ocorreu um erro interno. Tente novamente iniciando com /stats.")
        except:
...
def main():
    ...
    if client:
        ...
        logging.warning("Job Queue de atualização desativado: Conexão GSheets falhou.")
