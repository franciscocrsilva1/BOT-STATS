...
async def listar_competicoes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(0, len(abas_list), 3):
        ...
        for aba in abas_list[i:i + 3]:
            # CORREÇÃO CRÍTICA AQUI: Usa pipe (|) como separador
            # Antes: row.append(InlineKeyboardButton(aba, callback_data=f"c:{aba}")) 
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if tipo_comando == "c": # Ex: c|BSA
            ...
            if len(args) < 4:
                await query.edit_message_text("Erro: Comando 'AÇÃO' sem argumentos suficientes.")
                return
            acao_tipo, liga, mandante, visitante = args
            
            # O fluxo de RESULT agora cai diretamente no filtro 0.
            if acao_tipo == "STATS":
        ...
        # --- FLUXOS DE VOLTAR ---
            ...
            # Comando desconhecido, mostra o dado (que é exatamente o erro que você está vendo: c:CL)
