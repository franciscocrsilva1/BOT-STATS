...
async def mostrar_menu_acoes(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, mandante: str, visitante: str):
    """ Quarta tela: Menu para escolher o filtro de Estatísticas/Resultados. CORREÇÃO: SEMPRE envia o menu de filtros como uma NOVA MENSAGEM. """
...
async def exibir_estatisticas(update: Update, context: ContextTypes.DEFAULT_TYPE, mandante: str, visitante: str, aba_code: str, filtro_idx: int):
    """ Exibe as estatísticas detalhadas. CORREÇÃO DE UX: Envia o resultado como NOVA MENSAGEM e REEXIBE o menu. """
...
def main():
    ...
    if client:
        ...
        # Usa app.loop para agendar o pre-carregamento de cache de forma segura no loop de eventos
        app.loop.create_task(pre_carregar_cache_sheets())
    else:
        logging.warning("Job Queue de atualização desativado: Conexão com GSheets não estabelecida.")
    
    logging.info("Bot rodando...")
    # Correção: O método run_polling() já contém o loop de eventos, mas em ambientes
    # assíncronos como o Railway/container, é melhor ter certeza de que o loop
    # do JobQueue está sendo executado corretamente, o que já está implícito
    # no uso do ApplicationBuilder do python-telegram-bot.
    app.run_polling()
