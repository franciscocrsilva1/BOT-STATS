# ===============================================================================
# 🏆 BOT DE ESTATÍSTICAS DE CONFRONTO V2.2.0 - FIX UX: MENU FIXO E NOVA MENSAGEM
# ===============================================================================
...
# Filtros reutilizáveis para Estatísticas e Resultados (Mapeamento para Mandante e Visitante)
CONFRONTO_FILTROS = [
    # Filtro para Mandante | Filtro para Visitante
    (f"📊 Estatísticas | ÚLTIMOS {ULTIMOS} GERAL", "STATS", ULTIMOS, None, None),
    (f"📊 Estatísticas | {ULTIMOS} (M CASA vs V FORA)", "STATS", ULTIMOS, "casa", "fora"),
    (f"📅 Resultados | ÚLTIMOS {ULTIMOS} GERAL", "RESULTADOS", ULTIMOS, None, None),
    (f"📅 Resultados | {ULTIMOS} (M CASA vs V FORA)", "RESULTADOS", ULTIMOS, "casa", "fora"),
]
...
# (Mantidas as funções auxiliares: safe_int, pct, media, escape_markdown, get_sheet_data, get_sheet_data_future, pre_carregar_cache_sheets)
# =================================================================================
def safe_int(v):
    try: return int(v)
    except: return 0
...
# (Mantidas as funções: buscar_jogos, buscar_jogos_live, atualizar_planilhas)
# =================================================================================
...
# (Mantidas as funções: calcular_estatisticas_time, formatar_estatisticas, listar_ultimos_jogos)
# =================================================================================
...
# (Mantidas as funções: start_command, listar_competicoes, mostrar_menu_status_jogo, listar_jogos)
# =================================================================================
...
# (Volta para a lógica de exibir os filtros diretamente)
# =================================================================================
async def mostrar_menu_acoes(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, mandante: str, visitante: str):
    ...
    CORREÇÃO: Mostra os filtros diretamente, sem a seleção intermediária do time.
    """
    ...
    for idx, (label, tipo, ultimos, condicao_m, condicao_v) in enumerate(CONFRONTO_FILTROS):
        
        # O callback agora inclui Mandante e Visitante para o cálculo final
        callback_data = f"{tipo}_FILTRO|{aba_code}|{m_sanitized}|{v_sanitized}|{idx}"
...
async def exibir_estatisticas(update: Update, context: ContextTypes.DEFAULT_TYPE, mandante: str, visitante: str, aba_code: str, filtro_idx: int):
    ...
    # 2. Fecha o relógio de loading do botão (sem pop-up) para que o menu permaneça clicável
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # 6. Voltar para Menu de Ações (Volta para o menu de Filtros, que é a mesma função de mostrar ações)
