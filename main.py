...
def get_sheet_data_future(aba_code):
    ...
    if not linhas_raw or len(linhas_raw) <= 
1:
        return []
...
def buscar_jogos_live(league_code):
    ...
    try:
        ...
        logging.error(f"Erro ao buscar jogos AO VIVO (busca por 
data) para {league_code}: {e}")
        return []
...
async def atualizar_planilhas(context: ContextTypes.DEFAULT_TYPE):
    ...
    for aba_code, aba_config in LIGAS_MAP.items():
        ...
        try: ws_past = sh.worksheet(aba_past)
            ...
            logging.warning(f"Aba de histórico '{aba_past}' não encontrada.
Ignorando...")
            continue
        ...
        if jogos_finished:
            try:
                ...
                keys_exist = {(r['Mandante'], r['Visitante'], 
r['Data']) for r in exist}
                ...
                if novas_linhas:
                    ...
                    logging.info(f"✅ {len(novas_linhas)} jogos adicionados ao histórico 
de {aba_past}.")
...
def formatar_estatisticas(d):
    ...
    return (f"📊 **Estatísticas - {escape_markdown(d['time'])}**\n"
            ...
            f"⏱️ 1ºT Over 0.5: {pct(d['over05_1T'], jt)} (C: 
{pct(d['over05_1T_casa'], jc)} | F: {pct(d['over05_1T_fora'], jf)})\n"
            ...
            f"➖ **Média gols sofridos:** {media(d['gols_sofridos'], jt)} (C: {media(d.get('gols_sofridos_casa',0), jc)} |
F: {media(d.get('gols_sofridos_fora',0), jf)})\n\n"
...
async def mostrar_menu_status_jogo(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str):
    ...
    try:
        ...
        logging.error(f"ERRO ao 
editar mensagem em mostrar_menu_status_jogo (c|{aba_code}): {e}")
...
async def listar_jogos(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, status: str):
    ...
    if status == "FUTURE":
        ...
        try:
            ...
            logging.error(f"Erro ao editar mensagem 
de loading FUTURE: {e}")
            pass 
...
not jogos_agendados:
        ...
        for jogo in jogos_a_listar:
            ...
            try:
                ...
                label = 
f"({matchday_num}) {data_label} | {M_safe} x {V_safe}"
                ...
                if len(callback_data.encode('utf-8')) > 64:
                    ...
                    label = f"({matchday_num}) {data_label} |
{M_safe_short} x {V_safe_short}"
...
de loading LIVE: {e}")
        ...
        for jogo in jogos_a_listar:
            ...
            if len(callback_data.encode('utf-8')) > 64:
                ...
                label = f"🔴 {tempo} |
{M_safe_short} {placar_m} x {placar_v} {V_safe_short}"
...
# =================================================================================
async def mostrar_menu_acoes(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, mandante: 
str, visitante: str):
    """
    ...
    # Este 'answer()' é crucial quando a 
função é chamada após um clique em 'JOGO|...'
...
async def exibir_estatisticas(update: Update, context: ContextTypes.DEFAULT_TYPE, mandante: str, visitante: str, aba_code: str, filtro_idx: int):
    ...
    d_m = calcular_estatisticas_time(mandante, aba_code, 
ultimos=ultimos, casa_fora=condicao_m)
...
async def exibir_ultimos_resultados(update: Update, context: ContextTypes.DEFAULT_TYPE, mandante: str, visitante: str, aba_code: str, filtro_idx: int):
    ...
    texto_jogos_m = listar_ultimos_jogos(mandante, aba_code, 
ultimos=ultimos, casa_fora=condicao_m)
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Removido o query.answer() daqui para 
dar tempo da função ser chamada
    data = query.data
    ...
    try:
        ...
        # 3. Seleção de Jogo (JOGO|ABA_CODE|MANDANTE|VISITANTE) -> Vai direto para o menu de filtros
        if data.startswith("JOGO|"):
            ...
            # Chamada para a função que mostra o menu de filtros.
            await mostrar_menu_acoes(update, context, aba_code, mandante, visitante)
            return
        ...
        if data.startswith("VOLTAR_LIGA_STATUS|"): 
            _, aba_code = data.split('|')
            await 
mostrar_menu_status_jogo(update, context, aba_code)
            return
        ...
        try:
             await update.callback_query.edit_message_text("❌ Ocorreu um erro interno.
Tente novamente iniciando com /stats.")
        except:
