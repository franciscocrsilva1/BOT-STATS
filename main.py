...
def buscar_jogos(league_code, status_filter):
    ...
    if status_filter == "ALL":
        # Garante que apenas jogos agendados ou 
        # cronometrados (futuros) sejam retornados.
            ...
            if m.get('status') == "FINISHED":
                try:
                    ...
                    jogos.append({
                        ...
                        "Gols Mandante 2T": gm - gm1, "Gols Visitante 2T": gv 
                        - gv1,
         
                        "Data": jogo_data.strftime("%d/%m/%Y")
                    })
                except Exception: continue # CORREÇÃO: Usando except Exception
        return sorted(jogos, key=lambda x: datetime.strptime(x['Data'], "%d/%m/%Y"))

def buscar_jogos_live(league_code):
    """Busca jogos AO VIVO (IN_PLAY, HALF_TIME, PAUSED) buscando todos os 
    jogos do dia na API."""
...
async def atualizar_planilhas(context: ContextTypes.DEFAULT_TYPE):
    """Atualiza o 
    histórico e o cache de futuros jogos. Função para o JobQueue."""
    global SHEET_CACHE
    ...
    for aba_code, aba_config in LIGAS_MAP.items():
        ...
        # Pausa para respeitar limite de rate da API
                ...
                for m in jogos_future:
                    ...
                    if utc_date:
                        ...
                        try:
                            ...
                            # Limita a busca a jogos de 
                            # até 90 dias no futuro
                        ...
                        except Exception: # CORREÇÃO: Usando except Exception
                            continue
...
def calcular_estatisticas_time(time, aba, ultimos=None, casa_fora=None):
    ...
    for linha in linhas:
        ...
        d["over25_casa" if em_casa else "over25_fora"] += (1 if total 
        > 2.5 else 0)
...
def formatar_estatisticas(d):
    ...
    if jt == 0: return f"⚠️ **Nenhum 
    jogo encontrado** para **{escape_markdown(d['time'])}** com o filtro selecionado."
    ...
    return (f"📊 **Estatísticas - {escape_markdown(d['time'])}**\n"
            ...
            f"🥅 **G.A.T.
            # (Gol em Ambos os Tempos): {pct(d.get('g_a_t',0), jt)}** (C: {pct(d.get('g_a_t_casa',0), jc)} | F: {pct(d.get('g_a_t_fora',0), jf)})\n\n" # LINHA G.A.T. ADICIONADA
            ...
            f"⏱️ 2ºT Over 1.5: {pct(d['over15_2T'], jt)} (C: {pct(d['over15_2T_casa'], jc)} | F: 
            # {pct(d['over15_2T_fora'], jf)})\n\n"
            ...
            f"🔢 **Média total de gols:** {media(d['total_gols'], jt)} (C: {media(d.get('total_gols_casa',0), jc)} |
            # F: {media(d.get('total_gols_fora',0), jf)})"
    )
...
time com filtros."""
    try: 
        linhas = get_sheet_data(aba)
    except Exception: # CORREÇÃO DE SINTAXE (Linha 402)
    ...
    for l in linhas:
        data = l['Data']
        gm, gv = 
        safe_int(l['Gols Mandante']), safe_int(l['Gols Visitante'])
...
async def listar_competicoes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    title = "📊 **Estatísticas de Confronto:** Escolha a 
    # Competição:"
...
async def mostrar_menu_status_jogo(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str):
    """
    Segundo menu: Escolhe entre Jogos AO VIVO e 
    # Próximos Jogos (Future).
    """
    ...
    try:
        ...
        await update.callback_query.message.reply_text( # Fallback para o caso de erro de edição (e.g., mensagem muito antiga)
            title, 
...
async def listar_jogos(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, status: str):
    ...
    if status == "FUTURE":
        ...
        if not jogos_agendados:
            ...
            # Em vez de reply_text, edita para evitar a segunda mensagem se a edição acima funcionar.
            # Se a edição falhar, o callback é respondido abaixo (após o return)
            reply_markup = InlineKeyboardMarkup(keyboard)
            try:
        ...
        for jogo in jogos_a_listar:
            try:
                ...
                try:
                    ...
                    # Converte para UTC-3 (Horário de Brasília)
                ...
                # Lógica para encurtar o callback_data se for muito longo (limite Telegram: 64 bytes)
                if len(callback_data.encode('utf-8')) > 64:
                    M_safe_short = escape_markdown(M_full.split(' ')[0][:8])
                    V_safe_short = escape_markdown(V_full.split(' ')[0][:8])
        ...
        if not jogos_a_listar:
            ...
            # Edita ou envia o menu de voltar
            reply_markup = InlineKeyboardMarkup(keyboard)
            try:
                 await update.effective_message.edit_reply_markup(reply_markup=reply_markup)
            except:
    ...
    # Adiciona botão de voltar após a listagem de jogos (para ambos LIVE e FUTURE)
    keyboard.append([InlineKeyboardButton("⬅️ Voltar para Status", callback_data=f"VOLTAR_LIGA_STATUS|{aba_code}")])

    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # Edita a mensagem de "Buscando..." para a lista final de jogos
    await update.callback_query.edit_message_text(
...
# =================================================================================
# 🔄 FUNÇÕES DE FLUXO E CALLBACKS
# =================================================================================

async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Controlador central de callbacks para navegação no bot.
    Formato do callback_data: TIPO|PARAM1|PARAM2|...
    """
    query = update.callback_query
    await query.answer() # Responde imediatamente ao clique
    ...
    # STATUS|LIVE/FUTURE|ABA_CODE
    if data.startswith("STATUS|"):
        _, status, aba_code = data.split("|")
        await listar_jogos(update, context, aba_code, status)
        return

    # VOLTAR_LIGA_STATUS|ABA_CODE (Volta para a escolha entre LIVE/FUTURE)
    if data.startswith("VOLTAR_LIGA_STATUS|"):
        _, aba_code = data.split("|")
        await mostrar_menu_status_jogo(update, context, aba_code)
        return

    # JOGO|ABA_CODE|MANDANTE|VISITANTE
    if data.startswith("JOGO|"):
        try:
            _, aba_code, mandante, visitante = data.split("|")
        except ValueError:
            # Caso os nomes tenham sido encurtados
            _, aba_code, mandante, visitante = data.split("|")
            
        await mostrar_menu_acoes(update, context, aba_code, mandante, visitante)
        return

    # STATS_FILTRO/RESULTADOS_FILTRO|ABA_CODE|MANDANTE|VISITANTE|IDX_FILTRO
    if data.startswith("STATS_FILTRO|") or data.startswith("RESULTADOS_FILTRO|"):
        
        try:
            _, aba_code, mandante, visitante, idx_filtro = data.split("|")
        except ValueError:
             # Lida com possível erro de split em nomes encurtados que tenham "|"
             await update.callback_query.edit_message_text(f"❌ Erro ao processar o filtro. Tente novamente.")
             return

        try:
            idx = int(idx_filtro)
            _, tipo_filtro, ultimos, condicao_m, condicao_v = CONFRONTO_FILTROS[idx]
        except (ValueError, IndexError):
             await update.callback_query.edit_message_text(f"❌ Filtro inválido.")
             return

        if tipo_filtro == "STATS_FILTRO":
            await exibir_estatisticas(update, context, aba_code, mandante, visitante, ultimos, condicao_m, condicao_v)
        elif tipo_filtro == "RESULTADOS_FILTRO":
            await exibir_resultados(update, context, aba_code, mandante, visitante, ultimos, condicao_m, condicao_v)
        return
    
    # Voltar para o menu principal de ações (Filtros)
    if data.startswith("VOLTAR_MENU_ACOES|"):
        _, aba_code, mandante, visitante = data.split("|")
        await mostrar_menu_acoes(update, context, aba_code, mandante, visitante)
...
async def mostrar_menu_acoes(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, mandante: str, visitante: str):
    ...
    title = f"Escolha o filtro para o confronto **{m_sanitized} x {v_sanitized}** (Liga: {aba_code}):"
    ...
    # É importante responder ao callback para fechar o relógio 
    # de loading no botão clicado (JOGO|...)
    # Este 'answer()' é crucial quando a função é chamada após um clique em 'JOGO|...'
    # Já respondido no callback_query_handler
...
# =================================================================================
async def exibir_estatisticas(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, mandante: str, visitante: str, ultimos: int, condicao_m: str, condicao_v: str):
    """
    Calcula e exibe as estatísticas para Mandante e Visitante, 
    seguindo os filtros.
    """
    
    # 1. Busca estatísticas para o Mandante
    d_m = calcular_estatisticas_time(mandante, aba_code, ultimos, condicao_m)
    msg_m = formatar_estatisticas(d_m)

    # 2. Busca estatísticas para o Visitante
    d_v = calcular_estatisticas_time(visitante, aba_code, ultimos, condicao_v)
    msg_v = formatar_estatisticas(d_v)

    # 3. Formata a mensagem final
    message = (
        f"**🏆 Confronto: {escape_markdown(mandante)} x {escape_markdown(visitante)} ({aba_code})**\n"
        f"================================\n\n"
        f"{msg_m}\n\n"
        f"================================\n\n"
        f"{msg_v}"
    ...
    # 4. Envia a nova mensagem
    await update.effective_message.reply_text(message, parse_mode='Markdown')

    # 5. Re-envia o menu de filtros (ações)
    await mostrar_menu_acoes(update, context, aba_code, mandante, visitante)


# =================================================================================
# ✅ FUNÇÃO CORRIGIDA: EXIBIÇÃO DOS RESULTADOS
# Envia como NOVA MENSAGEM e reexibe o menu de filtros.
# =================================================================================
async def exibir_resultados(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, mandante: str, visitante: str, ultimos: int, condicao_m: str, condicao_v: str):
    """
    Lista os últimos jogos de Mandante e Visitante, seguindo os filtros.
    """
    
    # 1. Lista últimos jogos para o Mandante
    jogos_m = listar_ultimos_jogos(mandante, aba_code, ultimos, condicao_m)
    
    # 2. Lista últimos jogos para o Visitante
    jogos_v = listar_ultimos_jogos(visitante, aba_code, ultimos, condicao_v)

    # 3. Determina o título dos filtros
    filtro_label = f"ÚLTIMOS {ultimos}"
    if condicao_m == "casa" and condicao_v == "fora":
        filtro_label += " (M CASA vs V FORA)"
    elif condicao_m == "casa":
        filtro_label += " (JOGOS EM CASA)"
    elif condicao_v == "fora":
        filtro_label += " (JOGOS FORA)"

    # 4. Formata a mensagem final
    message = (
        f"**📅 Resultados - {filtro_label}**\n"
        f"**🏆 Confronto: {escape_markdown(mandante)} x {escape_markdown(visitante)} ({aba_code})**\n"
        f"================================\n\n"
        f"**{escape_markdown(mandante)}**:\n"
        f"```{jogos_m}```\n\n"
        f"================================\n\n"
        f"**{escape_markdown(visitante)}**:\n"
        f"```{jogos_v}```"
    )
    ...
    # 6. Re-envia o menu de filtros (ações)
    await mostrar_menu_acoes(update, context, aba_code, mandante, visitante)
...
def main():
    ...
    if client:
        ...
        # Ajustado para first=5 para dar tempo de inicializar o bot antes da primeira run
        job_queue.run_repeating(atualizar_planilhas, interval=3600, first=5, name="AtualizacaoPlanilhas")
    ...
    logging.info("Bot Iniciado e escutando...")
    app.run_polling()
