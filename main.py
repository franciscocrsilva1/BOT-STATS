...
async def pre_carregar_cache_sheets():
    ...
    # Sua lógica real de carregamento de planilhas deve estar aqui
    pass
    
async def atualizar_periodicamente(context: ContextTypes.DEFAULT_TYPE):
    # Sua lógica de atualização periódica deve estar aqui (ex: buscar jogos live, recalcular estatísticas)
...
# =================================================================================
# ⚙️ FUNÇÃO PRINCIPAL (Polling) - CORREÇÃO FINAL
# =================================================================================
def main():
    try:
        ...
        # 1. Cria o JobQueue primeiro (sem argumentos, CORREÇÃO JobQueue.__init__)
        job_queue = JobQueue() 
        
        # 2. Cria o ApplicationBuilder, INJETANDO o JobQueue
        # CORREÇÃO CRÍTICA (Setter): Isso resolve o erro 'has no setter'
        application = ApplicationBuilder().token(BOT_TOKEN).job_queue(job_queue).build()
        #                                                  ^^^^^^^^^^^^^^^^^^^
        
        # 3. Inicia o JobQueue (Agora ele está dentro da Application)
        job_queue.start() 
